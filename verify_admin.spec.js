...
test('Verify Admin Panel dynamic logs', async ({ page }) => {
  ...
  await expect(page.locator('div:has-text("Indexed 3 new remote engineering roles.")').first()).toBeVisible();
  await expect(page.locator('div:has-text("Web3 Data Update [2026-03-30] complete.")').first()).toBeVisible();

  // Scroll to logs
  await logsHeading.scrollIntoViewIfNeeded();
